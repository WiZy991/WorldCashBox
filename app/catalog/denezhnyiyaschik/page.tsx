...
export default function CashDrawersPage() {
	...
	const filteredProducts = products.filter(product => {
		// Фильтруем по категории equipment и подкатегории drawers, или по ключевым словам
		const isCashDrawer =
			product.category === 'equipment' &&
			(product.subcategory === 'drawers' ||
				product.name.toLowerCase().includes('денежный ящик') ||
				product.name.toLowerCase().includes('денежные ящики') ||
				product.description.toLowerCase().includes('денежный ящик') ||
				product.description.toLowerCase().includes('денежные ящики'))
		...
		return isCashDrawer && matchesSearch
	})
		...
		try {
			const response = await fetch('/api/sbis/submit', {
				...
				body: JSON.stringify({
					...
					message: 'Заказ денежных ящиков',
					product: 'Денежные ящики',
				}),
			})
				...
				<div className='container mx-auto px-4 relative z-10'>
					...
					>
						...
						<p className='text-xl text-primary-100 mb-8'>
							Надежные денежные ящики для кассовых систем
						</p>
						<div className='bg-white/10 backdrop-blur-md rounded-2xl p-4 mb-6'>
							<p className='text-sm text-primary-100'>
								⚠️ Цены в каталоге не являются окончательными и могут меняться в
								зависимости от курса доллара
							</p>
							<p className='text-sm text-primary-100 mt-2'>
								📞 Наши менеджеры подберут для Вас лучшее предложение по
								телефону{' '}
								<a href='tel:+74232799759' className='font-bold underline'>
									+7 (423) 2-799-759
								</a>
							</p>
						</div>
					...
					<div className='md:col-span-2'>
						<div className='bg-white rounded-2xl shadow-xl p-6'>
							<div className='relative'>
								...
								<input
									type='text'
									placeholder='Поиск денежных ящиков...'
									value={searchQuery}
					...
					<div className='bg-white rounded-2xl shadow-xl p-6'>
						...
						<form onSubmit={handleOrderSubmit} className='space-y-4'>
							...
							<p className='text-xs text-gray-500 text-center'>
								Нажимая кнопку «Оформить заказ» вы соглашаетесь с политикой в
				...
				{loading ? (
					...
					<div className='text-center py-12 bg-white rounded-2xl shadow-xl'>
						<p className='text-gray-600 text-lg'>
							Товары не найдены. Свяжитесь с нами для получения информации о
							товарах.
						</p>
					</div>
				)}
